...
fn find_kth_positive(arr: Vec<i32>, k: i32) -> i32 {
    ...
    while left < right {
        ...
        let missing_nums = arr[mid] - mid as i32 - 1;
    ...
    let missing_nums = arr[left] - left as i32 - 1;

    if missing_nums == k {
        return arr[left] - 1;
    } else if missing_nums < k {
        return arr[left] + (k - missing_nums);
    } else {
        return arr[left] - 1 - (missing_nums - k);
    }
}
